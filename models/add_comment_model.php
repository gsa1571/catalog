...
function add_comment(){
    ...
    if(empty($comment_author) OR empty($comment_text)){
        $res = ['answer' => 'Все поля обязательны к заполнению'];
        return json_encode($res);
    }

    $query = "INSERT INTO comments (comment_author, comment_text, parent, comment_product, is_admin) VALUES ('$comment_author', '$comment_text', $parent, $comment_product, $is_admin)";
...
function get_last_comment($comment_id){
    ...
    $res = ['answer' => 'Коментарий добавлен', 'code' => $comment_html, 'id' => $comment_id];
    return json_encode($res);
}
